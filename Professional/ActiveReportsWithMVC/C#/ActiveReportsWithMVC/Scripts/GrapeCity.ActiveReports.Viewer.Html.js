!function(window) {
    ...
    function ar_convertClientParameter(p) {
        ...
        return p.multivalue ? {
            ...
            SelectAll: p.value === ParameterSpecialValue.SelectAll || p.values[0].value === ParameterSpecialValue.SelectAll || void 0
        } : {
            Name: p.name,
    ...
    function ArsReportService(options, resourceManager) {
        ...
        function sendImpl(token, settings) {
            ...
            return $base.sendEmail(description, newsettings).then(function(url) {
        ...
        function toggle(token, toggleInfo, settings) {
            ...
            return options && options.previewPages && (renderOptions.ExtensionSettings.PreviewPages = options.previewPages), 
    ...
    function DocumentModel(reportService, token, reportType, resourceManager) {
        ...
        function sendImpl(settings) {
            ...
            return reportService.sendEmail(token, settings).fail(failHandler);
        }
        var nullToc = {
            kids: []
        }, emptyToc = {
    ...
    function DocumentViewModel(viewer) {
        ...
        viewer.pageDisplayMode.subscribe(function() {
            ...
            ko.waitFor(doc.state, function(val) {
                ...
                val === DocumentState.completed && setInteractivity(!0);
            });
        });
        var _inProgress = ko.computed(function() {
            return setInteractivity(viewer.document().state() === DocumentState.completed && !viewer.report().busy()), 
            viewer.report().busy();
    ...
    function ParameterViewModel(services, param, parameterChanged, parameterUpdateIsValid, reportModel) {
        ...
        function convertToTyped(value, type) {
            switch (value = isNullOrUndefined(value) ? null : value, type) {
              case ParameterType.Bool:
              ...
              default:
                if (null === value && !_parameterModel.nullable || "" === value && !_parameterModel.allowEmpty) throw new Error(getResourceString("error.ExpectValue"));
            }
            return value;
        }
        ...
        function convertToString(value) {
            if (!_nullCheckboxValue && null === value) {
        ...
        function validateAndConvertValue(value) {
            _hideError = !1, value = isNullOrUndefined(value) ? null : value;
            try {
        ...
        function isPredefinedValue(value) {
            ...
            return value = isNullOrUndefined(value) ? null : value, void 0 !== _parameterModel.availableValues && (_parameterModel.availableValues.map(function(v) {
                return AsString(v.value);
            }).indexOf(AsString(value)) !== -1 || (!!(_parameterModel.availableValues.length > 0 && _parameterModel.nullable && null === value) || !!(_parameterModel.availableValues.length > 0 && _parameterModel.selectAllEnabled && value === ParameterSpecialValue.SelectAll)));
        }
        function updateValue(newValue) {
            newValue = isNullOrUndefined(newValue) ? null : newValue, setIsValueNull(newValue), 
            _parameterModel.type == ParameterType.DateTime & (checkDateFormatYearOnly(newValue) | checkDateFormat(newValue)) & !checkYear(newValue) || (newValue = validateAndConvertValue(newValue), 
    ...
    function ReportServiceSelector(options, resourceManager) {
        ...
        if (!options.url) throw new Error("options has no valid url");
        var impl, api = {};
        return [ "open", "close", "run", "validateParameters", "validateParameter", "getPageCount", "getPage", "getToc", "export", "search", "drillthrough", "toggle", "ping", "sendEmail", "canSendEmail", "getState", "setState", "getDelayedContent" ].forEach(function(method) {
    ...
    function sendEmailDialogViewModel(services, viewer) {
        ...
        _chekEmails = function() {
            ...
            return emails.forEach(function(item) {
                return "" === item ? res : null !== item && (/^[0-9a-z-\.]+\@[0-9a-z-]{1,}\.[a-z]{1,}$/i.test(item) || (res = !1), 
                res);
    ...
    function SinglePageViewModel(viewer) {
        function getPageContent() {
            viewer.document().getPage(viewer.pageIndex()).done(_pageContent);
        }
            ...
            exportTo: function(exportType) {
                return {
                    exec: function() {
                        ...
                        viewer.document().export(exportType, options).done(function(exportUrl) {
                            if (exportUrl) {
        ...
        return {
            sendEmail: function() {
                ...
                viewer.document().sendEmail(settings).done(function() {
                    viewer.sidebarState(SidebarState.Hidden), $("#sendemaildialog").modal("hide"), _sendEmailDialog.errorResponse("");
                }).fail(function() {
                    var error = resolveErrorMessage(arguments, SR);
                    _sendEmailDialog.errorResponse(error);
                });
            },
            canSendEmail: function() {
    ...
    function($, window, undefined) {
        ...
        function Plugin(el, options) {
            ...
            base.isHeadSticky = !1, base.isHidden = !1, base.$floatContainer = $("<div>").css({
                overflow: "hidden",
                position: "fixed"
    ...
    PageDisplayMode.single = "single", PageDisplayMode.continuous = "continuous", function($) {
        ...
        update: function(element, valueAccessor, allBindings, viewModel, bindingContext) {
            ...
            void 0 !== settings.page() && (element.innerHTML = settings.page()), $(element).ready(function() {
                var zIndex = 50;
