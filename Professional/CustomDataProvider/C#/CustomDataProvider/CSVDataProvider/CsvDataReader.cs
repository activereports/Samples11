...
namespace GrapeCity.ActiveReports.Samples.CustomDataProvider.CsvDataProvider
{
	...
	{
        private Hashtable _typeLookup = new Hashtable(new myCultureComparer(new CultureInfo("en")));
		...
		private void ParseCommandText()
		{
            ...
            try
            {
                if (!ParseHeader(_textReader.ReadLine()))
					throw new InvalidOperationException("Field names and types are not defined. The first line in the CommandText must contain the field names and data types. e.g FirstName(string)");
            }
            catch (Exception)
            {
            }
		...
		private bool ParseDataRow(string dataRow)
		{
			...
			{
                string error = string.Format(CultureInfo.InvariantCulture, "Invalid row \"{0}\". The row does not contain the same number of data columns as the table header definition.", dataRow);
		...
		private object ConvertValue(Type type, string originalValue)
		{
			...
			{
                throw new InvalidOperationException(string.Format("Input value '{0}' could not be converted to the type '{1}'.", originalValue, type), e);
			}
